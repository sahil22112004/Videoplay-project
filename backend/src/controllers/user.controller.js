    ...
    if (userA._id.toString() === userB._id.toString()) {
        return res.status(400).json(new ApiResponse(400, {}, "You cannot subscribe to yourself"));
    }

    // Prevent duplicate subscription
    if (userB.subscribedBy.map(id => id.toString()).includes(userA._id.toString())) {
        return res.status(400).json(new ApiResponse(400, {}, "You are already subscribed to this user"));
    }

    // Update userB (channel owner)
    userB.subscribers += 1;
    ...
    const isSubscribed = userB.subscribedBy
        .map(id => id.toString())
        .includes(userA._id.toString());
    ...
    // Remove from userB's subscribedBy
    ...
    // Remove from userA's subscribedChannels
    userA.subscribedChannels = userA.subscribedChannels.filter(
